    ...
    public function index(Request $request){
        ...
        if(requestUrl::segment(3) !== null){
            ...
            if( DB::table('products')->where('kategoria', $subcategory)->where('w_sprzedazy', 1)->orderBy('id', 'desc')->get()){
                $products = DB::table('products')->where('kategoria', $subcategory)->where('w_sprzedazy', 1)->orderBy('id', 'desc')->paginate(32);
            }
        ...
        if(requestUrl::segment(3) !== 'sort' ){
            ...
            if ($sortBy == 'new') {
                ...
                if( DB::table('products')->where('kategoria', $subcategory)->where('w_sprzedazy', 1)->orderBy('cena', 'asc')->paginate(32)){
                    $products = DB::table('products')->where('kategoria', $subcategory)->where('w_sprzedazy', 1)->orderBy('cena', 'asc')->paginate(32);
                    $select = 'down';
                }
            } else if($sortBy == 'up'){
                if( DB::table('products')->where('kategoria', $subcategory)->where('w_sprzedazy', 1)->orderBy('cena', 'desc')->paginate(32)){
                    $products = DB::table('products')->where('kategoria', $subcategory)->where('w_sprzedazy', 1)->orderBy('cena', 'desc')->paginate(32);
                    $select = 'up';
                }
            } else {
                if( DB::table('products')->where('kategoria', $subcategory)->where('w_sprzedazy', 1)->orderBy('ilosc_kupionych', 'desc')->paginate(32)){
                    $products = DB::table('products')->where('kategoria', $subcategory)->where('w_sprzedazy', 1)->orderBy('ilosc_kupionych', 'desc')->paginate(32);
                    $select = 'most';
                }
            }
            ...
            if ($sortBy == 'new') {
                ...
                if( DB::table('products')->where('rodzaj', $category)->where('w_sprzedazy', 1)->orderBy('cena', 'asc')->paginate(32)){
                    $products = DB::table('products')->where('rodzaj', $category)->where('w_sprzedazy', 1)->orderBy('cena', 'asc')->paginate(32);
                    $select = 'down';
                }
            } else if($sortBy == 'up'){
                if( DB::table('products')->where('rodzaj', $category)->where('w_sprzedazy', 1)->orderBy('cena', 'desc')->paginate(32)){
                    $products = DB::table('products')->where('rodzaj', $category)->where('w_sprzedazy', 1)->orderBy('cena', 'desc')->paginate(32);
                    $select = 'up';
                }
            } else {
                if( DB::table('products')->where('rodzaj', $category)->where('w_sprzedazy', 1)->orderBy('ilosc_kupionych', 'desc')->paginate(32)){
                    $products = DB::table('products')->where('rodzaj', $category)->where('w_sprzedazy', 1)->orderBy('ilosc_kupionych', 'desc')->paginate(32);
                    $select = 'most';
                }
            }
