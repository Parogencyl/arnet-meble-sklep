        ...
        <div class="col-12">
            <div class="row">
                        ...
                        <div class="row p-0 m-0 font-weight-bold justify-content-center">
                            ...
                            <div class="col-md-1 col-11 mt-2 mt-md-0 d-flex justify-content-center align-items-center">
                                <form action="/product/deleteFromCart" method="POST" class="bg-inherit">
                                    @csrf
                                    <input type="text" name="kategoria_id" value="{{ $product->id }}" class="d-none">
                ...
                <div class="col-12 text-md-right text-center font-weight-bold py-3 mt-4 pr-md-5 pr-0"
                    ...
                    koszt: <span id="total">
                        {{ $total }} </span> zł </div>
                <p class="mb-0 other"> Zalecane jest dokończenie zamówienia. Dopóki produkt nie zostanie zakupiony
                    przez
                    Ciebie, inni mogą
                    zamówić ostatnie sztuki danego produktu. </p>
                <p class="other"> Wszystkie zamówienia wysyłane są za pomocą firmy kurierskiej. </p>

                <form action="/summaryCart" method="POST" class="col-12">
                    @csrf
                    @for ($i = 0; $i < count(session()->get('cart')); $i++)
                        <input type="text" name="{{ 'amount'.$i }}" value="{{ $sessionProducts[$i]->amount }}"
